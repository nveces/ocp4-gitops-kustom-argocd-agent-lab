...
== 🚀 PoC Wizard: ArgoCD Agent Managed Mode
...
* **`argocd`** - https://github.com/argoproj-labs/argocd-agent[window=_blank].
...
│   └── images/
├── managed/
│   ├── control-plane/             # Control-plane cluster configuration
│   │   ├── 10-base/               # Namespaces base
│   │   ├── 20-principal/          # ArgoCD Principal Server
│   │   │   ├── 10-argocd.yml
│   │   │   ├── 20-svc-proxy-service.yml
│   │   │   ├── 30-app-project.yml
│   │   │   ├── 40-route.yml
│   │   │   └── kustomization.yaml
│   │   └── 30-apps/
│   │       └── managed-cluster/   # Applications for the managed cluster
│   │           └── 10-apps-bgd.yml
│   └── managed-cluster/           # Managed cluster configuration
│       ├── 10-argocd/             # ArgoCD con server deshabilitado
│       │   ├─ 10-base/
│       │   |  ├── 10-ns-argocd-agent.yml
│       │   |  └── kustomization.yaml
│       │   └─ 20-argocd-instance/
│       |      ├── 20-argocd.yml   # ArgoCD Instance with controller disabled
│       |      └── kustomization.yaml
│       └── 20-agent/              # ArgoCD Agent
│           └── 10-base/
│               ├── 10-agent-redis-networkpolicy.yml
│               ├── 20-application-controller-cluster-admin-role.yml
│               └── kustomization.yaml.tpl
├── operator/                     # Operator GitOps installation
│   ├── 10-base/
│   |   ├── 10-ns-project.yml
│   |   └── kustomization.yaml
│   └── 20-operator/
